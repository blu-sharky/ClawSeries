...
【语言规则 - 必须遵守】
你必须使用与用户消息相同的语言进行回复。如果用户使用英文，你必须用英文回复；如果用户使用中文，你必须用中文回复。开场白、问题文本、选项文本都必须使用用户的语言。

背景约束：
1. 这是 AI 短剧，不是传统长剧；后续会进入 AI 分镜、资产生成和视频生成流程。
2. 问题应帮助模型产出更适合 AI 生成的内容：人物关系清晰、场景集中、钩子强、反转快。
3. 后续视频生成按 Veo 3.1 的 8 秒单镜头限制执行，提问和方案设计时必须主动控制单场信息量、动作量和换景频率。
4. 如果用户已经给出集数、单集时长、目标观众，就不要再问这些。
5. 不要问预算。
...
2. 如果是 select，选项要有明显区分度。"""
        ...
        try:
            data = None
            system_msg = {"role": "system", "content": "你是一个专业的 AI 短剧制片顾问。后续分镜与视频生成按 8 秒单镜头限制执行，你的追问和建议必须服务于这个限制。\n\n【输出规则 - 绝对遵守】\n1. 直接输出纯 JSON 对象，不要包裹在 markdown 代码块中。\n2. 禁止输出任何 JSON 以外的内容：不要解释、不要注释、不要前后缀文字。\n3. 禁止使用 ```json ``` 包裹。"}
...
【语言规则 - 必须遵守】
你必须使用与用户原始请求相同的语言生成大纲。如果用户原始请求是英文，大纲必须用英文；如果是中文，大纲必须用中文。

项目要求：
1. 这是 AI 短剧，节奏要快，每集都应有明确钩子或反转。
2. 角色关系要清晰，核心场景尽量集中，便于后续 AI 分镜、资产和视频生成。
3. 后续视频生成按 8 秒单镜头限制执行，剧情设计必须保证单镜头内动作和信息密度可落地。
4. 如果用户已经明确给出集数、单集时长、目标观众，就严格以用户输入为准。
...
- 每个分镜镜头必须按 8 秒上限来写，避免一个镜头承载过多人物、事件或空间变化
用户需求：
- 类型：{genre}
...
后续视频生成按 8 秒单镜头限制执行，最终汇报版大纲也必须控制单镜头的信息量、动作量和换景频率。
用户需求：
- 类型：{genre}
- 建议集数：{episode_count}集
- 建议单集时长：{episode_duration}
...
请直接开始输出最终大纲内容。"""
    ...
    async def _generate_outline_with_llm(self, collected: dict) -> ScriptOutline:
        ...
        system_msg = {"role": "system", "content": "你是一个专业的 AI 短剧编剧。后续分镜与视频生成按 8 秒单镜头限制执行，你生成的大纲必须服务于这个限制。\n\n【输出规则 - 绝对遵守】\n1. 直接输出纯 JSON 对象，不要包裹在 markdown 代码块中。\n2. 禁止输出任何 JSON 以外的内容：不要解释、不要注释、不要前后缀文字。\n3. 禁止使用 ```json ``` 包裹。"}
    ...
    async def stream_next_questions(
        ...
        instruction = (
            ...
            '已明确的信息不要重复问。严禁询问预算、拍摄成本、演员资源、置景成本。'
            '后续分镜与视频生成按 8 秒单镜头限制执行，问题必须帮助用户把内容收敛到这个限制内。'
            '问题要像创作讨论，不要像制片表格。'
            '【语言规则】你必须使用与用户消息相同的语言进行回复。如果用户使用英文，你必须用英文回复；如果用户使用中文，你必须用中文回复。开场白、问题文本、选项文本都必须使用用户的语言。'
        )
